...
class Cell(object):
    ...
    Production : nltk.Production = None
    R = -1

    def __init__(self, production : nltk.Production, offset : int):
...
class CnfParser(object):
    ...
    def __is_in_here(self, nonterminal : nltk.Nonterminal, mySet : CustomSet) -> bool:
        if mySet is None:
            return False
        for item in mySet:
    ...
    def recursive_backtrace(self, lhs, table, r_i:int, c_i:int) -> []:
        results = []
        myCustomSet : CustomSet = table[r_i][c_i]
        cell : Cell
    ...
    def parse_all_sentences(self, lines):
        ...
        for line in lines:
            ...
            for interpretation in self.parse(tokens):
                interpretationAsTree = nltk.Tree.fromstring(interpretation)
                print(interpretationAsTree)
                no_of_parses += 1
